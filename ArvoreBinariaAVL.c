...
tno *insere(tno *raiz, tno *novo){
	...
	case -2: if (raiz->esq->fb <=0)
					raiz=rotacaoDir(raiz); //rota?ao simples para a direita 
	...
	case 2: if (raiz->dir->fb >=0)
					raiz= rotacaoEsq(raiz); //rota?ao simples para a esquerda 
...
tno *removeNo(tno *r){
	...
	// percorrer at? achar o menor da direita
	pai = r; nova = r->dir;
	while(nova->esq!=NULL){
		pai=nova;
		nova = nova->esq;
...
tno *carrega(tno *raiz){
    ...
    for(i=0;i<n;i++){
        fscanf(arq,"%d",&chave);
		raiz=insere(raiz,criaNo(chave,dado)); // aqui este dado est? sendo inserido com nossa fun??o insere j? pronta
	}// fim for
	fclose(arq);
